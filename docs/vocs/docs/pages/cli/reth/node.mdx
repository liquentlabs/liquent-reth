...
Liquent:
      ...
      --liquent.disable-levm
          Disable the Levm executor. default false

      --liquent.cache.max-persist-gap <CACHE_MAX_PERSIST_GAP>
          The max block height between merged and pesist block height

          [default: 128]

      --liquent.persist.merge-blocks
          Persist consecutive blocks in merged groups to amortize per-block fsyncs (much faster from-genesis catch-up). Incompatible with Storage V2. default false

      --liquent.cache.capacity <CACHE_CAPACITY>
          The max size of cached items

          [default: 2000000]

      --liquent.report-db-metrics
...
Storage:
      --storage.v2 [<V2>]
          ...
          When set, new databases are initialized with account/storage changesets stored in static file segments instead of the state database. Existing databases always use the settings persisted in their metadata regardless of this flag.
