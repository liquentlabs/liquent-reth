...
use reth_chainspec::{
    is_liquent_system_caller, is_system_tx_gas_exempt, ChainSpecProvider, EthChainSpec,
    EthereumHardforks,
};
    ...
    pub fn new(
        ...
        executor.spawn_task(async move {
            while let Some(event) = stream.next().await {
                ...
                {
                    bad_block_store.insert(recovered, "invalid block".to_string());
                }
            }
        });
        ...
        self.eth_api()
            .spawn_with_state_at_block(block.parent_hash(), move |eth_api, mut db| {
                ...
                while let Some((index, tx)) = transactions.next() {
                    ...
                    let tx_sender_is_system_caller = is_liquent_system_caller(tx.signer());
    ...
    pub async fn debug_trace_transaction(
        ...
        self.eth_api()
            .spawn_with_state_at_block(state_at, move |eth_api, mut db| {
                ...
                ) && is_liquent_system_caller(tx.signer())
                {
