...
/// Canonical sender address of every Liquent protocol-injected system transaction
...
/// Protocol-reserved synthetic emitter for `LiquentTxSkipped` receipt logs.
///
/// This is not an EVM account or precompile. It is reserved in the Liquent system-address
...
/// gas-exempt gating; all callsites — serial executor, levm executor, pipe
...
pub fn is_system_tx_gas_exempt<S: EthChainSpec>(chain_spec: &S, block_ts: u64) -> bool {
    chain_spec.liquent_hardforks().is_fork_active_at_timestamp(LiquentHardfork::Alpha, block_ts)
}
...
/// [`LiquentHardfork::Beta`] is active at `block_ts` (genesis `betaTime`), the
...
pub fn is_eip7702_lockdown_active<S: EthChainSpec>(chain_spec: &S, block_ts: u64) -> bool {
    !chain_spec.liquent_hardforks().is_fork_active_at_timestamp(LiquentHardfork::Beta, block_ts)
}
    ...
    I: IntoIterator<Item = Address>,
{
    ...
    for (idx, addr) in senders.into_iter().enumerate() {
        if is_liquent_system_caller(addr) {
            if saw_non_system {
                return Err(idx);
            }
        } else {
