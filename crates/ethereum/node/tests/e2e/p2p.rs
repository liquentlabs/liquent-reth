...
// Liquent's consensus is BFT-final: persistence is forward-only and the state cache enforces a
// monotonic persist-block invariant. reth's native deep-reorg drives persistence backwards, which
// that invariant forbids by design, so this upstream reorg test is inapplicable to liquent.
#[tokio::test]
#[ignore = "liquent BFT persistence is forward-only; reth native deep-reorg unsupported by design"]
...
// See `test_long_reorg`: liquent's forward-only BFT persistence invariant is incompatible with
