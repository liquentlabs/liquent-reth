...
use liquent_primitives::PIPE_BLOCK_GAS_LIMIT;
use levm::{DelegatedSafetyConfig, DynParallelPrecompile};
use reth_chain_state::{ExecutedBlockWithTrieUpdates, ExecutedTrieUpdates};
use reth_chainspec::{ChainSpec, EthChainSpec, EthereumHardforks, LiquentHardfork};
...
use crate::{
    ...
    randomness_precompile::{ExecutionRandomnessProvider, LiquentStorageRandomnessProvider},
};

fn extract_liquent_events_from_system_receipts(
    receipts: &[Receipt],
    block_number: u64,
    epoch: u64,
) -> Vec<LiquentEvent> {
    use liquent_api_types::on_chain_config::jwks::ProviderJWKs;

    let mut liquent_events = vec![];
    ...
    for receipt in receipts {
        debug!(target: "execute_ordered_block",
            ...
            "extract liquent events from receipt"
        );
        for log in &receipt.logs {
            ...
            if log.address == DKG_ADDR {
                if let Ok(event) = DKGStartEvent::decode_log(&log) {
                    ...
                    liquent_events.push(LiquentEvent::DKG(convert_dkg_start_event_to_api(&event)));
                }
            }
        }
    }
    ...
    if !data_records.is_empty() {
        let api_jwks: Vec<ProviderJWKs> = data_records
            ...
            .map(|((source_type, source_id), nonce)| {
                // issuer format: "liquent://sourceType/sourceId"
                let issuer = format!("liquent://{}/{}", source_type, source_id);
                ProviderJWKs {
                    issuer: issuer.into_bytes(),
                    version: nonce as u64, // nonce as version
                    jwks: vec![liquent_api_types::on_chain_config::jwks::JWKStruct {
                        // laptos reads liquent:// oracle progress from Any.data as a raw
                        // 16-byte nonce. The LiquentEvent adapter only preserves raw bytes
        ...
        liquent_events.push(LiquentEvent::ObservedJWKsUpdated(epoch, api_jwks));
    }
...
mod tests {
    ...
    #[test]
    fn extract_liquent_events_ignores_data_recorded_from_wrong_emitter() {
        ...
        let events = extract_liquent_events_from_system_receipts(&forged_receipts, 10, 7);
        assert!(events.is_empty(), "forged DataRecorded emitter must not produce LiquentEvent");

        let valid_receipts = vec![receipt_with_log(NATIVE_ORACLE_ADDR, event.encode_log_data())];
        let events = extract_liquent_events_from_system_receipts(&valid_receipts, 10, 7);
        assert_eq!(events.len(), 1);
        match &events[0] {
            LiquentEvent::ObservedJWKsUpdated(epoch, jwks) => {
                assert_eq!(*epoch, 7);
    ...
    #[test]
    fn extract_liquent_events_accepts_oracle_delivered_from_native_oracle_only() {
        ...
        let events = extract_liquent_events_from_system_receipts(&forged_receipts, 10, 7);
        assert!(events.is_empty(), "forged OracleDelivered emitter must not produce LiquentEvent");
    ...
    #[test]
    fn extract_liquent_events_ignores_dkg_start_from_wrong_emitter() {
        ...
        let events = extract_liquent_events_from_system_receipts(&forged_receipts, 10, 7);
        assert!(events.is_empty(), "forged DKGStartEvent emitter must not produce LiquentEvent");

        let valid_receipts = vec![receipt_with_log(DKG_ADDR, event.encode_log_data())];
        let events = extract_liquent_events_from_system_receipts(&valid_receipts, 10, 7);
        assert_eq!(events.len(), 1);
        assert!(matches!(events[0], LiquentEvent::DKG(_)));
    }
}
...
pub struct OrderedBlock {
    /// Epoch of the block
    pub epoch: u64,
    /// `BlockId` of the parent block generated by Liquent SDK
    pub parent_id: B256,
    /// `BlockId` of the block generated by Liquent SDK
    pub id: B256,
...
/// Owned by EL
#[derive(Debug)]
struct PipeExecService<Storage: LiquentStorage> {
    ...
    /// Receive the execution init args from `LiquentSDK`
...
pub struct ExecutionResult {
    ...
    /// This is the block id generated by Liquent SDK
    pub block_id: B256,
    /// Block number of the executed block
    pub block_number: u64,
    ...
    /// Liquent events emitted by the executed block
    pub liquent_events: Vec<LiquentEvent>,
}
...
#[derive(Debug)]
struct Core<Storage: LiquentStorage> {
...
impl<Storage: LiquentStorage> PipeExecService<Storage> {
...
struct ExecuteOrderedBlockResult {
    ...
    txs_info: Vec<TxInfo>,
    liquent_events: Vec<LiquentEvent>,
    epoch: u64,
}
...
impl<Storage: LiquentStorage> Core<Storage> {
    fn epoch(&self) -> u64 {
    ...
    fn custom_precompiles_for_ordered_block(
        ...
        if !self
            .chain_spec
            .liquent_hardforks()
            .is_fork_active_at_timestamp(LiquentHardfork::Alpha, block_timestamp)
        {
            return self.pre_alpha_precompiles.clone()
        }

        let canonical_provider = LiquentStorageRandomnessProvider::new(self.storage.clone());
        let gas_policy =
    ...
    /// `verify_executed_block_hash` in this function are intentional. In the liquent-sdk
    ...
    async fn process(&self, block: ReceivedBlock) {
        ...
        if !self
            .chain_spec
            .liquent_hardforks()
            .is_fork_active_at_timestamp(LiquentHardfork::Alpha, block.header.timestamp)
        {
        ...
        let execution_result =
            ExecutionResult { block_id, block_number, epoch, block_hash, txs_info, liquent_events };
    ...
    fn create_block_for_executor(
        ...
        // validator receipts are appended. Closes liquent-audit#621.
        sum_system_gas: u64,
    ...
    /// Extract liquent events from execution receipts
    /// Returns liquent_events containing DKG events and ObservedJWKsUpdated from system-contract
    /// receipts. Emitter addresses are checked here so the function remains safe even if its
    /// caller's receipt slice changes.
    fn extract_liquent_events_from_receipts(
        &self,
        receipts: &[Receipt],
        block_number: u64,
        epoch: u64,
    ) -> Vec<LiquentEvent> {
        extract_liquent_events_from_system_receipts(receipts, block_number, epoch)
    }
    ...
    fn execute_ordered_block(
        ...
        let evm_env = self
            ...
            .next_evm_env(
                ...
                &NextBlockEnvAttributes {
                    ...
                    // `next_evm_env` only reads env-relevant fields; liquent assembles its own
                    // headers in the pipeline, so extra_data here is inert. slot_number is
                    // post-Amsterdam (liquent does not activate Amsterdam).
        ...
        let is_alpha_active = self.chain_spec.liquent_hardforks().is_fork_active_at_timestamp(
            LiquentHardfork::Alpha,
            ordered_block.timestamp_us / 1_000_000,
        );

        // Liquent Alpha (system-tx gas-exempt) boundary state change: zero
        ...
        // Liquent Alpha hardfork: gas-exempt system transactions (L2 —
        ...
        // receipts get appended below. Closes liquent-audit#621.
        ...
        // Liquent events are mined from SYSTEM_CALLER protocol txs only (metadata +
        ...
        // Only extract liquent events from protocol system transaction receipts.
        let liquent_events = self.extract_liquent_events_from_receipts(
            &result.execution_output.receipts[..n_protocol_system_receipts],
            result.block.number,
            epoch,
        );

        result.liquent_events.extend(liquent_events);
        result
    }
    ...
    /// in-memory** except for (3). `LiquentStorage` performs no disk I/O, so on
    /// crash and restart the reth DB is the sole source of truth. There is no
    /// "split-brain" risk between `LiquentStorage` and the reth DB because
    /// `LiquentStorage` state does not survive a restart.
...
impl<Storage: LiquentStorage, EthApi> PipeExecLayerApi<Storage, EthApi> {
