...
use liquent_api_types::events::contract_event::LiquentEvent;
...
pub(crate) fn system_txns_into_executed_ordered_block_result(
    ...
    ExecuteOrderedBlockResult {
        ...
        txs_info: vec![],
        liquent_events: vec![LiquentEvent::NewEpoch(new_epoch, validators.into())],
        epoch: new_epoch,
    }
}
...
mod tests {
    ...
    fn system_txns_into_epoch_change_result_preserves_completed_prefix() {
        ...
        assert!(matches!(result.liquent_events.as_slice(), [LiquentEvent::NewEpoch(4, _)]));
    }
}
