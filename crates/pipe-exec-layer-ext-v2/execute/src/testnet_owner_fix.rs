...
use reth_chainspec::{ChainSpec, EthChainSpec, LiquentHardfork, LONGEVITY_TESTNET_CHAIN_ID};
use reth_evm::{
...
mod tests {
        ...
        match (v1_ts, v2_ts) {
            (Some(v1), Some(v2)) => {
                spec.liquent_hardforks = ChainHardforks::from([
                    (LiquentHardfork::TestnetOwnerFix, ForkCondition::Timestamp(v1)),
                    (LiquentHardfork::TestnetOwnerFixV2, ForkCondition::Timestamp(v2)),
                ]);
            }
            (Some(v1), None) => {
                spec.liquent_hardforks = ChainHardforks::from([(
                    LiquentHardfork::TestnetOwnerFix,
                    ForkCondition::Timestamp(v1),
                )]);
            }
            (None, Some(v2)) => {
                spec.liquent_hardforks = ChainHardforks::from([(
                    LiquentHardfork::TestnetOwnerFixV2,
    ...
    fn wrong_chain_id_is_noop_even_on_crossing_block() {
        let mut spec = ChainSpecBuilder::from(&*MAINNET)
            ...
            .build();
        spec.liquent_hardforks = ChainHardforks::from([
            (LiquentHardfork::TestnetOwnerFix, ForkCondition::Timestamp(FIX_TS)),
            (LiquentHardfork::TestnetOwnerFixV2, ForkCondition::Timestamp(FIX_V2_TS)),
        ]);
    ...
    fn post_activation_block_is_noop_even_when_fork_active() {
        ...
        assert!(spec
            .liquent_hardforks()
            .is_fork_active_at_timestamp(LiquentHardfork::TestnetOwnerFixV2, FIX_V2_TS + 1));
        assert!(!spec
            .liquent_hardforks()
            .fork(LiquentHardfork::TestnetOwnerFixV2)
    ...
    fn v2_crossing_is_recognized_after_v1_already_active() {
        ...
        assert!(spec
            .liquent_hardforks()
            .is_fork_active_at_timestamp(LiquentHardfork::TestnetOwnerFix, FIX_V2_TS));
        assert!(!spec
            .liquent_hardforks()
            .fork(LiquentHardfork::TestnetOwnerFix)
