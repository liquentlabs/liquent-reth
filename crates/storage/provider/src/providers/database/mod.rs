...
use reth_db_api::{
    database::Database,
    models::{LiquentStorageSettings, StoredBlockBodyIndices},
    tables,
    transaction::DbTx,
};
...
pub struct ProviderFactory<N: NodeTypesWithDB> {
    ...
    storage_settings: Arc<RwLock<LiquentStorageSettings>>,
}
...
fn load_storage_settings<DB: Database>(db: &DB) -> LiquentStorageSettings {
    db.tx()
        .ok()
        .and_then(|tx| {
            tx.get::<tables::Metadata>(metadata_keys::LIQUENT_STORAGE_SETTINGS.to_string())
                .ok()
                .flatten()
        })
        .and_then(|bytes| LiquentStorageSettings::from_metadata_bytes(&bytes))
        .unwrap_or_else(LiquentStorageSettings::legacy)
}
...
impl<N: ProviderNodeTypes> BlockNumReader for ProviderFactory<N> {
    ...
    fn recover_block_number(&self) -> ProviderResult<BlockNumber> {
        self.provider()?.recover_block_number()
    }
