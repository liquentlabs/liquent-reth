...
impl<N: NodePrimitives> StaticFileProvider<N> {
        ...
        N: NodePrimitives<Receipt: Value, BlockHeader: Value, SignedTx: Value>,
    {
        ...
        for segment in StaticFileSegment::iter().filter(|s| {
            // liquent keeps transaction senders in the database and never has this segment;
            ...
            if let Some(unwind) = match segment {
                ...
                // liquent does not maintain storage-v2 static file segments (filtered above)
                _ => None,
            } {
        ...
        Provider: DBProvider + BlockReader + StageCheckpointReader,
    {
        ...
        let checkpoint_block_number = provider
            .get_stage_checkpoint(match segment {
                ...
                // liquent does not maintain storage-v2 static file segments (filtered above)
                _ => unreachable!("liquent static files only cover Headers/Transactions/Receipts"),
            })?
            .unwrap_or_default()
            .block_number;
